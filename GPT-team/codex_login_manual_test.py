...
def resolve_joini_cloud_otp_token(email: str, config: Optional[Dict[str, Any]] = None) -> str:
    """读取 joini.cloud 自动收码所需的邮箱访问令牌。AI by zb"""
    if not is_joini_cloud_email(email):
        return ""

    temp_mail_cfg = (config or {}).get("temp_mail") or {}
    if not isinstance(temp_mail_cfg, dict):
        return ""
    return str(temp_mail_cfg.get("admin_password") or "").strip()


def perform_http_oauth_login_manual(
    module,
    email: str,
    password: str,
    proxy: str = "",
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        otp_token = resolve_joini_cloud_otp_token(email, config)
        auto_otp_enabled = bool(otp_token)
        baseline_mail_ids: set = set()
        mail_trigger_time = None
        if auto_otp_enabled:
            baseline_mail_ids = module.snapshot_mail_identities(email, otp_token)
            mail_trigger_time = module.dt.datetime.now(module.MAIL_TIMEZONE)
        if auto_otp_enabled:
            module.logger.info("[ManualCodex] Step D: joini.cloud 邮箱自动填写 OTP | email=%s", email)
        elif is_joini_cloud_email(email):
            module.logger.warning(
                "[ManualCodex] Step D: joini.cloud 邮箱缺少 temp_mail.admin_password，回退手动输入 | email=%s",
                email,
            )
        else:
            module.logger.info("[ManualCodex] Step D: 非 joini.cloud 邮箱，等待手动输入 OTP | email=%s", email)
        ...
        otp_poll_started_at = module.time.time()
        while module.time.time() < verify_deadline:
            if auto_otp_enabled:
                latest_candidate = None
                all_emails = module.fetch_emails_list(email, otp_token)
                for item in module.sort_mail_items_newest_first(all_emails):
                    if not isinstance(item, dict):
                        continue
                    mail_time = module.parse_mail_item_time(item)
                    is_new_by_time = bool(mail_time and mail_trigger_time and mail_time > mail_trigger_time)
                    identity = module.build_mail_item_identity(item)
                    if identity and identity in baseline_mail_ids and not is_new_by_time:
                        continue
                    if mail_time and mail_trigger_time and mail_time <= mail_trigger_time:
                        continue
                    latest_candidate = item
                    break
                if not latest_candidate:
                    if not fallback_otp_sent and module.time.time() - otp_poll_started_at >= OTP_FALLBACK_SEND_DELAY_SECONDS:
                        try:
                            resp_send = session.get(
                                f"{module.OPENAI_AUTH_BASE}/api/accounts/email-otp/send",
                                headers=h_val,
                                verify=False,
                                timeout=30,
                            )
                            module.logger.info("[ManualCodex] OTP fallback send: HTTP %s | email=%s", resp_send.status_code, email)
                            fallback_otp_sent = True
                        except Exception as e:
                            module.logger.warning("[ManualCodex] OTP fallback send 异常: %s | email=%s", e, email)
                    module.time.sleep(2)
                    continue

                candidate_identity = module.build_mail_item_identity(latest_candidate)
                if candidate_identity and candidate_identity == last_attempted_identity:
                    module.time.sleep(2)
                    continue

                otp_code = module.extract_otp_from_mail_item(latest_candidate)
                if not otp_code:
                    module.time.sleep(2)
                    continue

                last_attempted_identity = candidate_identity or f"code:{otp_code}"
                ...
                if resp_val.status_code == 200:
                    try:
                        ...
                        pass
                    module.logger.info(
                        "[ManualCodex] OTP 验证成功 | continue_url=%s | page_type=%s | email=%s",
                        continue_url[:120],
                        page_type,
                        email,
                    )
                ...
                module.logger.warning(
                    "[ManualCodex] OTP 验证失败: HTTP %s | %s",
                    resp_val.status_code,
                    resp_val.text[:200],
                )
                if resp_val.status_code == 400 and "max_check_attempts" in resp_val.text:
                    module.logger.warning("[ManualCodex] OTP 验证次数已达上限，停止继续尝试 | email=%s", email)
                    return None
